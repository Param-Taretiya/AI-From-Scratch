...
gradiants=[]
momentum=[[[0]*785 for _ in range(16)],  # Input → HL1
    [[0]*17  for _ in range(16)],   # HL1 → HL2
    [[0]*17  for _ in range(10)] # HL2 → output
    ]                               
second_moment=[[[0]*785 for _ in range(16)],  # Input → HL1
    [[0]*17  for _ in range(16)],   # HL1 → HL2
    [[0]*17  for _ in range(10)]]  # HL2 → output
    ...
    for i in range(3):
        for j in range(len(G[i])):
            for k in range(len(G[i][j])):
                ...
                momentum[i][j][k]=m
                second_moment[i][j][k]=v
                
                m_hat,v_hat=bias_correction(m,v)
                
                if k!=len(G[i][j])-1:
                    weights_list[i][j][k]-= lr*(m_hat/(math.sqrt(v_hat)+e))
                else:
                    bias_list[i][j][0]-= lr*m_hat/(math.sqrt(v_hat)+e)
